...
class Constraints_generator():
    ...
    def genConstraints_of_forwardRound(self, r):
        ...
        for i in range(8):
            constr = constr + MITMPreConstraints.twoXOR_forward(input1_round[i],input2_round[i],TK1_1[i],TK1_2[i],TK2_1[i],TK2_2[i],\
            input1_sr[i],input2_sr[i],CD_ART[i])
            constr = constr + MITMPreConstraints.cd_dual_forward_one(input1_round[i],input2_round[i],CD_ART[i],CD_ART_dual[i])
    ...
    def genConstraints_of_backwardRound(self, r):
        ...
        #- Constraints for  ART (input1, input2, TK1_1, TK2_1, TK1_2, TK2_2, TK1_3, TK2_3, inter1_var, inter2_var, output1, output2, cd): 
        for i in range(8):
            constr = constr + MITMPreConstraints.twoXOR_backward(input1_sr[i],input2_sr[i],TK1_1[i],TK1_2[i],TK2_1[i],TK2_2[i],\
            input1_round[i],input2_round[i],CD_ART[i])
            constr = constr + MITMPreConstraints.cd_dual_backward_one(input1_sr[i],input2_sr[i],CD_ART[i],CD_ART_dual[i])
    ...
    def genConstraints_additional(self):
        ...
        if len(CD2_b_TK_dual) > 0:
            constr = constr + ['GDeg1' + ' - ' + BasicTools.MinusTerm(d1_TK1 + d1_TK2) + ' + ' + BasicTools.plusTerm(CD2_b_TK_dual) + ' = 0']
            if len(CD2_b + CD2_b_TK) > 0:
                constr = constr + [BasicTools.plusTerm(d1) + ' - ' + BasicTools.MinusTerm(CD2_b + CD2_b_TK) + ' + ' + BasicTools.plusTerm(CD2_b_TK_dual) + ' >= 0']
            else:
                constr = constr + [BasicTools.plusTerm(d1) + ' + ' + BasicTools.plusTerm(CD2_b_TK_dual) + ' >= 0']
        else:
            constr = constr + ['GDeg1' + ' - ' + BasicTools.MinusTerm(d1_TK1 + d1_TK2)  + ' = 0']
            if len(CD2_b + CD2_b_TK) > 0:
        ...
        if len(CD1_f_TK_dual) > 0:
            constr = constr + ['GDeg2' + ' - ' + BasicTools.MinusTerm(d2_TK1 + d2_TK2) + ' + ' + BasicTools.plusTerm(CD1_f_TK_dual) + ' = 0'] 
            if len(CD1_f + CD1_f_TK) > 0:
                constr = constr + [BasicTools.plusTerm(d2) + ' - ' + BasicTools.MinusTerm(CD1_f + CD1_f_TK) + ' + ' + BasicTools.plusTerm(CD1_f_TK_dual) + ' >= 0']
            else:
                constr = constr + [BasicTools.plusTerm(d2) + ' + ' + BasicTools.plusTerm(CD1_f_TK_dual) + ' >= 0']            
        else:
            constr = constr + ['GDeg2' + ' - ' + BasicTools.MinusTerm(d2_TK1 + d2_TK2) + ' = 0']
            if len(CD1_f + CD1_f_TK) > 0:
...
def cmd():
    rd = open('./skinny/Result_256_18.txt', 'w')
    rd.write('TR, ini_r, ini_k, mat_r: d1, d2, m' + '\n' )
    for TR in range(18, 19):
